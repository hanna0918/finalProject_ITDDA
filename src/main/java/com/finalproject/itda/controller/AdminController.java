...
//-------------------------
   ...
   //신고 게시판 모달에 출력   
         ...
         @ResponseBody public BoardVO CallBoardView(BoardVO vo) {
               System.out.println(vo.getM_seq()); 
...
//-----
      ...
      //블럭게시물 모달에 출력
      @ResponseBody
      @RequestMapping(value="/BoardAllViewmodal", method=RequestMethod.POST)
      public BoardVO AdminAllModal(BoardVO vo) {
         System.out.println(vo.getM_seq()); 
         System.out.println("모든 페이지 모달 들어옴!! 갓 순찬"); 
         BoardVO boardvo = adminService.Allboardmodal(vo);
         return boardvo; 
         }
